...
Use this naive method to transform pairs of `~` delimiters into HTML `<del>…</del>` elements before processing the
Markdown. After processing, the `<del>` and `</del>` tags simply become raw HTML tokens:

~~~ php
$value = preg_replace('/(~{1,2})(?!\s)(.+?)(?<!\s)\1/s', '<del>$2</del>', $value);

echo from($value);
~~~

The previous method will blindly parse the [strike syntax](https://github.github.com/gfm#example-491) in codes and
trailing raw HTML block fragments before the blank line or the end of the stream, which should be left as they are. If
you want a more native functionality, you can implement [this method](x/strike.php).
...
if ($value && false !== strpos($value, '</h')) {
    static $f = [];
    $value = preg_replace_callback('/<(h[2-6])(\s(?>"[^"]*"|\'[^\']*\'|[^>])*)?>(.+?)<\/\1>/s', static function ($m) use (&$f) {
