...
if ('LICENSE' === $test) {
    ...
    $files = glob(PATH . D . 'test' . D . $test . D . $batch . D . '*.md', GLOB_NOSORT);
}
...
$s .= '</p>';
$s .= '<p role="group">';
foreach (array_merge(glob(PATH . D . 'test' . D . '*', GLOB_ONLYDIR), ['LICENSE', 'README']) as $v) {
...
$folders = glob(PATH . D . 'test' . D . $test . D . '*', GLOB_ONLYDIR);
...
foreach ($files as $file) {
    ...
    if ('result' === $view) {
        $start = hrtime(true);
        $r = x\markdown\f($raws, ['block' => $block]) ?? "";
